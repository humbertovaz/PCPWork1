...
int nbx, bx, nby, by;
  ...
  bx = M/nbx + ((M%nbx) ? 1 : 0); // linha do bloco (quantas linhas fica cada th
read)
  ...
  wtime = omp_get_wtime();
    ...
    while(iter<ITER){
        #pragma omp parallel for  //reduction(+:sum) private(diff) //  i -> linh
as; j colunas; ii-> chunk atual de linhas; jj-> chunk atual  de colunas
        for (int ii=0; ii<nbx; ii++)  // Criar #nbx threads    
            for (int jj=0; jj<nby; jj++)   // Limitar chunksize por thread
                for (int i=1+ii*bx; i<=min((ii+1)*bx, M-2); i++)       // cada i
 é uma linha do bloco
                    for (int j=1+jj*by; j<=min((jj+1)*by, N-2); j++) {    // cad
a j é uma coluna do bloco
